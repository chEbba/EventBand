...
namespace Che\EventBand\Tests\Routing;

use Che\EventBand\Routing\EventPatternRouter;
...
class EventPatternRouterTest extends TestCase
{
     ...
     *
     * @expectedException Che\EventBand\Routing\EventRoutingException
     * @expectedExceptionMessage is not a scalar
     */
    public function routeReplaceNonScalars()
    {
        $router = new EventPatternRouter('{propagationStopped}');
        $event = $this->getMock('Symfony\Component\EventDispatcher\Event');
        $event
            ->expects($this->once())
